...
5. **Test integrity audit** (if this phase includes new or modified test files):
    ...
    - **Prop-through tautology check (component tests only):** For each component test file, scan for the pattern where a test passes a prop and then asserts that exact value appears in the DOM with no conditional logic or computation involved. Detection: look for `getByText(VARIABLE)` or `getByText(STRING_LITERAL)` where STRING_LITERAL matches a prop value set in the same test, and no `if`/conditional logic is being tested. Flag each instance — these tests provide false confidence.
    - **Render-vs-interaction ratio (component tests only):** Count render-only assertions (prop appears, child component renders, CSS class present) versus behavior assertions (click handlers, callback firing, state transitions, conditional branches). If render-only exceeds 50% of assertions in a file, flag it as inverted priority.
    - **API wrapper value check:** For test files covering API modules (`*-api.test.*`), verify each tested function contains non-trivial logic (data transformation, conditional branching, error parsing beyond `if (!response.ok) throw`). If the function is a pure `fetch → json` pass-through with no logic, flag the test as negative value.
    - **Callback wiring verification (component tests):** For any test whose name includes "passes", "wires", "calls", or "propagates" a callback: verify the test actually triggers the interaction that should fire the callback (click, submit, select) and asserts the callback was called with correct arguments. If the test only checks that an element exists without interacting with it, flag as misleading.
    - **Shape-only assertions:** Grep for `toHaveProperty(` in test files. For each match, check whether the test also asserts the expected VALUE of that property (not just its existence). If a test only checks shape (property exists) without value, flag as weak assertion.
