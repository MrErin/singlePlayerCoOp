...
| ------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.                                          | `/plan:interrogate`                                             | Talks to you about your idea and builds a structured requirements doc                                                             | Can also be invoked after requirements exist in order to discuss new features or clarify things.                     | Depends on the complexity of the idea. I've had good results starting with GLM and then doing a final, shorter, analysis with Opus to catch gotchas. |
...
| 4.                                          | `/plan:build [X]`                                               | Builds according to the skills and the plan                                                                                       | Uses `code-reviewer` agents per-task to verify that the task is complete                                             | GLM                                                                                                                                                  |
...
| After full buildout (or after a TEST phase) | `/plan:debt`                                                    | Assesses entire codebase against `my-style` standards and lists issues to address in order of severity, along with time estimates |                                                                                                                      | Opus (though GLM is pretty good too)                                                                                                                 |
| After full buildout (or after a TEST phase) | `/plan:audit`                                                   | Combined debt + test audit in one pass. Creates a cluster deck grouping related issues together for efficient fixing              | Replaces running `/plan:debt` and `/plan:test-audit` separately (still deciding if I like them separate or together) | Opus                                                                                                                                                 |
...
| At the even bitter-er end                   | `/plan:archive`                                                 | Archives all feature-specific `_planning` documents, keeps the `project-requirements` files updated                               |                                                                                                                      | Sonnet                                                                                                                                               |

### Backlog Management

The backlog system lives in `_planning/backlog/`. It lets me dump random ideas, notes, bugs, etc. that I collect while using the app (or going for a walk or whatever) into an inbox as a disorganized mess. The agent can sort out the list into categories that allow us to pull specific ideas into features during `/plan:interrogate` phases.
