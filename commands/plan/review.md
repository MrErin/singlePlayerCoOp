...
6. **If `ua_testing.md` doesn't exist yet** (e.g., review was run before build completed): Generate it from the `ua_testing` template. Use **progressive disclosure** — put "Quick Smoke" tests first (5-10 min), then "Priority 1" features (15-20 min), then "Priority 2" if time permits. Don't overwhelm the user with one giant checklist.
7. **If ua_testing.md exists** (generated by `/plan:build` step 9): Ask the user what needs updating. Common scenarios:
